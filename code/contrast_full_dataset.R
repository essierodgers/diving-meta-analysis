...
plot(tree)
tree_noutliers <- read.tree("./data/phylo_outliers_removed.NWK")
plot(tree_noutliers)


# Create a phylogenetic correlation matrix
PhyloA <- vcv(tree, corr = TRUE)
PhyloA_outliers_removed <- vcv(tree_noutliers, corr = TRUE)
...
# Make shared control matrix for ROM dataset
  ...
  summary(model2.RR)  
  data_full_ROM %>% group_by(respiration_mode) %>% summarise(mean_t = mean(mean_t))
  ...
  #remove outlier- need to updated V afterwards
   data_full_ROM_outlier_removed <- data_full_ROM[-c(5,16,64),]

  
  #update V matrix
  V_no_outlier <- make_VCV_matrix(data_full_ROM_outlier_removed, "vi", "sc_cluster", 
                                  type = "vcv", rho = 0.5)
  
  #Tree checks_outliers_removed
  setdiff(unique(data_full_ROM_outlier_removed$species_rotl), sort(rownames(PhyloA_outliers_removed)))
 
  # Check same number of levels
  length(rownames(PhyloA_outliers_removed))
  length(unique(data_full_ROM_outlier_removed$species_rotl))
  ...
  model2.RR.nooutlier <- rma.mv(yi = yi, V = V_no_outlier, 
                                ...
                                data =  data_full_ROM_outlier_removed)
  ...
  model3.RR <- rma.mv(yi = yi, V = V_no_outlier, 
  ...
  model4.RR <- rma.mv(yi = yi, V = V_no_outlier, 
                      mods = ~ mean_t + delta_t + study_type-1, 
                      random = list(~1|study_ID, ~1|species_rotl, ~1|obs), 
                      R = list(species_rotl = PhyloA_outliers_removed), test = "t", 
                      data = data_full_ROM_outlier_removed)
  model4.RR <- rma.mv(yi = yi, V = V_no_outlier, 
  ...
  I2(model5.CVR, v = data_verts_CVR$vi, phylo = "species_rotl")  
