...
if(rerun_data == TRUE){
  ...
  data <- read.csv("./data/finalised_data_sets/contrast_field_dataset.csv", stringsAsFactors = FALSE)
  ...
  write.csv(data, "./data/data_field.csv", row.names=FALSE)
}else {
  data_field <- read.csv("./data/data_field.csv", stringsAsFactors=FALSE)
...
tree <- read.tree("./data/finalised_data_sets/phylo_field.NWK")
plot(tree)

tree2 <- read.tree("./data/finalised_data_sets/phylo_field_nooutlier.NWK")
plot(tree2)
...
setdiff(unique(data_field$species_rotl), sort(rownames(PhyloA)))
# Check same number of levels
length(rownames(PhyloA))
length(unique(data_field$species_rotl))



# Have a look at the mean-variance relationship
plot_func(data_field, "sd_t1", "mean_t1")
...
I2(model1.RR, v = data_field_ROM$vi, phylo = "species_rotl")
...
#remove outliers
data_field_ROM_outlier_removed <- data_field_ROM[-c(15,16),]

#update V matrix
V_no_outlier_ROM <- make_VCV_matrix(data_field_ROM_outlier_removed, "vi", "sc_cluster", 
                                type = "vcv", rho = 0.5)
...
model1.RR.nooutlier <- rma.mv(yi = yi, V = V_no_outlier_ROM, 
                    ...
                    data = data_field_ROM_outlier_removed)
...
I2(model1.RR.nooutlier, v = data_field_ROM_outlier_removed$vi, phylo = "species_rotl")


# Publication bias (Funnels):
res <- residuals(model1.RR.nooutlier)
funnel(res, vi = data_field_ROM_outlier_removed$vi, yaxis = "seinv")
...
I2(model4.CVR, v = data_field_CVR$vi, phylo = "species_rotl")  
...
#remove outliers
data_field_CVR_outlier_removed <- data_field_CVR[-c(15,16),]

#update V matrix
V_no_outlier_CVR <- make_VCV_matrix(data_field_CVR_outlier_removed, "vi", "sc_cluster", 
                                    type = "vcv", rho = 0.5)
...
#Cannot disentangle study effects from species effects
I2(model4.CVR.nooutlier, v = data_field_CVR_outlier_removed$vi, phylo = "species_rotl")  
...
funnel(res, vi = data_field_CVR_outlier_removed$vi, yaxis = "seinv")
