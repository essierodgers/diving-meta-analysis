...
model2_phylo <- rma.mv(yi ~ delta_t + log(body_mass_g), V = vi, random = list(~1|study_ID, ~1|species_rotl,  ~1|error), data = data_verts)

R = list(species_rotl = PhyloA)
...
model7 <- MCMCglmm(log(sd) ~ log(mean) + log(body_mass_g) + T, mev = arms_data2$sd_sv, random = ~us(1):study_ID + us(1+T):species, data = arms_data2, prior = prior_slope, nitt = 50000, burnin = 10000, thin = 30)
summary(model7)
